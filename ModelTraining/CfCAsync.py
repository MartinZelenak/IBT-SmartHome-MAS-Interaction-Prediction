...
class PerDeviceCfCAsync(nn.Module):

    def __init__(self, shared_input_size: int, per_device_input_size: int, n_devices: int, n_neurons: int, lookahead: int):
        """
        Initializes the PerDeviceCfCAsync model with specified parameters for shared and device-specific inputs.
        ...
        """
        super(PerDeviceCfCAsync, self).__init__()
...
if __name__ == "__main__":
    ...
    if load_model:
        model.load_state_dict(torch.load(f"{out_folder}/{CfC.__name__}.pth"))
    else:
        # Loss and optimizer
        ...
        # Save model
        print('Saving model...')
        torch.save(model.state_dict(), f"{out_folder}/{CfC.__name__}.pth")
        ## Save parameters

...
if __name__ == "__main__":
    ...
    max_samples = None
    sequence_len = 1  # Time sequence length
    time_step = 5 # minutes
    ...
    train_dataset = SequentialData.SequentialStateDataset(train_data_file_path, n_data_columns, max_samples, lookback=sequence_len, device=device)
    eval_dataset = SequentialData.SequentialStateDataset(eval_data_file_path, n_data_columns, max_samples)
    ...
    wiring = AutoNCP(n_neurons, output_size) # TODO: Too random? Does it have good results each time?
    model = PerDeviceLTC(
    ...
    if load_model:
        ...
        for epoch in range(n_epochs):
            ...
            for i, data in enumerate(train_dataset):    # type: ignore
                ...
                pred, hidden_state = model.forward(sequence.unsqueeze(0), hidden_state)
                hidden_state = hidden_state.detach()

                loss = criterion(pred[:, [-1], :], next_state[None, None])
    ...
    with torch.no_grad():
        ...
        for i, data in enumerate(eval_dataset): # type: ignore
            ...
            # Predict
            pred, _ = model.forward(sequence.unsqueeze(0), hidden_state)
    ...
    with open(f'{out_folder}/accuracy.txt', 'w') as f:
        ...
        to_print += f'Correct state change guesses with 1 sample lag: {sum(correct_device_state_changes_with_lag)}/{total_state_changes} ({(sum(correct_device_state_changes_with_lag)/total_state_changes)*100:.2f}%)\n'
        print(to_print, end="")
        f.write(to_print)
        for i in range(n_SmartDevices):
            ...
            to_print += f'Device {i} correct state change guesses with 1 sample lag: {correct_device_state_changes_with_lag[i]}/{devices_state_changes[i]} ({((correct_device_state_changes_with_lag[i]/devices_state_changes[i]) * 100):.2f}%)\n'
            print(to_print, end="")
            f.write(to_print)
    del i

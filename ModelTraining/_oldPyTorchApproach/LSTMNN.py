...
if __name__ == "__main__":
    ...
    n_SmartDevices = 8
    remove_feature_columns = ['DayOfMonth', 'Month', 'Year', 'Location'] # Remove unused feature columns
    max_samples = None
    ...
    transforms = [
        Data.RemoveColumnsTransform(
            remove_feature_cols=[
                column_name_number_map[col_name]
                for col_name in remove_feature_columns
            ],
            remove_label_cols=range(0, n_total_time_columns + 1),
        )
    ]
    ...
    model = LSTMNN(input_size=n_feature_columns,  
    ...
    # Save model
    print('Saving model...')
    torch.save(model.state_dict(), f'{outFolder}/{LSTMNN.__name__}.pth')
    print('Model saved')

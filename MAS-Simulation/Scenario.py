...
def setup_home(home: hm.Home):
    ...
    JID_device_dict[f'{device.name}@{MAS_JABBER_HOST}'] = device
    home.rooms['livingroom'].add_device(device)
...
def MAS_start():
    ...
    print(f'Connecting to TCP relay agent...')
    ...
    while True:
        try:
            ...
            time.sleep(1)
    print(f'Connected to TCP relay agent')
...
def MAS_send_message(message: spadeMessages.Message) -> spadeMessages.Message|None:
    ...
    '''
    global MAS_socket
    if not MAS_socket or not MAS_socket.fileno() != -1:
...
def main():
    try:
        ...
        print(f'KeyboardInterrupt: Stopping simulation...')
    except Exception as e:
        print(f'Error: {e}')
